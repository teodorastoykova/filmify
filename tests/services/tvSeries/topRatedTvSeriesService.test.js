...
import getTopRatedTvSeries from "../../../src/services/TvSeries/TopRatedTvSeriesService";
...
describe("getTopRatedTvSeries", async () => {
  it("Successfully fetches the data", async () => {
    ...
    const tvSeries = await getTopRatedTvSeries();
  ...
  it("Failes to fetch data", async () => {
    ...
    await expect(getTopRatedTvSeries()).rejects.toThrow("Rejected");

    expect(consoleErrorSpy).toHaveBeenCalledWith(
      "Failed to get the top rated TV Series",
      expect.any(Error)
    );
  });
});
